...
static BYTE read_8(buf_parser* p) {
    return (p->read_pos < p->size) ? p->buf[p->read_pos++] : 0;
}
...
static BOOL tpm_read_public_area(TBS_HCONTEXT h_tbs_context, UINT32 handle, BYTE** out_public, DWORD* out_public_size) {
    ...
    *out_public_size = 0;
    ...
    if (pub_size == 0 || p.read_pos + pub_size > resp_size) return FALSE;
...
static BYTE* ecc_to_bcrypt_blob(UINT16 curve_id, const BYTE* x, UINT16 x_size, const BYTE* y, UINT16 y_size, DWORD* out_blob_size) {
    DWORD magic = (curve_id == 0x0003) ? BCRYPT_ECDH_PUBLIC_P256_MAGIC : (curve_id == 0x0004 ? BCRYPT_ECDH_PUBLIC_P384_MAGIC : 0);
...
static BOOL execute_possession_challenge(TBS_HCONTEXT h_tbs_context, UINT32 ek_handle, UINT32 ak_handle) {
    ...
    return (validated && decrypted_size == 16 && memcmp(challenge, decrypted_challenge, 16) == 0);
}
...
BOOL perform_local_tpm_pop_challenge(PCCERT_CONTEXT ek_cert) {
    ...
    BOOL success = execute_possession_challenge(h_tbs_context, preinstalled_ek_handle, ak_handle);
...
static BOOL tpm_quote(TBS_HCONTEXT hContext, UINT32 akHandle, const BYTE* nonce, UINT16 nonceSize, BYTE* outAttest, UINT16* outAttestSize, BYTE* outSig, UINT16* outSigSize) {
    ...
    if (attestSize == 0 || p.read_pos + attestSize > respSize) return FALSE;
    ...
    if (sigSize == 0 || p.read_pos + sigSize > respSize) return FALSE;
...
static BOOL parse_and_verify_attest_structure(const BYTE* attestBytes, UINT16 attestSize, const BYTE* expectedNonce, UINT16 expectedNonceSize, BYTE* outPcrDigest, UINT16* outPcrDigestSize) {
    ...
    if (read_32(&p) != 1) return FALSE; // count
    if (read_16(&p) != TPM_ALG_SHA256) return FALSE;
    if (read_8(&p) != 3) return FALSE;
    if (read_8(&p) != 0xFE) return FALSE; // mask for PCRs 1..7

    read_8(&p); read_8(&p);

    UINT16 digestSize = read_16(&p);
    if (digestSize != 32 || p.read_pos + digestSize > attestSize) return FALSE;
